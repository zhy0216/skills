...
这里的“fork”是为每个任务创建独立 Git worktree 和独立 agent 实例；不要使用 CLI 的会话 fork 功能代替 Git worktree 隔离。
...
对仍处于 `working`/Thinking 的 agent 采用 **15 分钟监控节奏**：完成一次批量状态检查后，若没有 `done`、`idle` 或 `blocked` 等可操作状态，为**每个**在跑 agent 同时挂一个后台 wait 任务（各自 15 分钟封顶），任一任务先 settle 就立即唤醒——不要只盯着一个 agent 干等，让其他已完成的任务空转排队：
...
实现可以并行，但 rebase → 冲突处理 → 仓库级校验 → 合并必须持有单一“集成锁”，一次只处理一个任务。其他 agent 可继续实现，但不能同时推进原分支。

对待集成任务执行：

1. 再次确认原 checkout 干净，并记录原分支当前 HEAD。在整个集成阶段不要合并其他任务。
2. 提示同一个任务 agent 在自己的 worktree 中执行：
...
3. 报告：
   ...
   - 每个任务实际使用的 agent、模型、Codex 推理强度与仓库级校验证据；
