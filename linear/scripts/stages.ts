...
export type StageResult = {
  ...
  validationCommentId: string | null; planDocumentId: string | null; planUrl: string | null; summary: string;
};
...
export function parseStageResult(value: any, stage: Stage, issueId: string): StageResult {
  ...
  for (const key of ["commit", "validatedBaseSha", "validationCommentId", "planDocumentId", "planUrl"] as const) {
  ...
  if (value.outcome === "needs_validation" && stage !== "merge") throw new Error(`Only merge can request needs_validation`);
...
export function parseOrchestratorResult(value: any, issueId: string, allowedStages: Stage[], canComplete: boolean): OrchestratorResult {
  ...
  if (value.outcome === "dispatch") {
    ...
    if (value.outcome === "completed" && !canComplete) throw new Error("Orchestrator cannot complete the issue before a verified merge");
  }
  return value;
}
...
export async function runStages(context: RunContext, agents: RoleAgents, options: {
  ...
  const availableStages = (): Stage[] => {
    ...
    return ["merge", "validate", "implement", ...planning];
  };
  ...
  const requestValidation = () => {
    ...
    baseChanges++;
    reviewReason = "The base branch advanced; dispatch executor/validate again before merging.";
  ...
  for (let turn = 1; turn <= MAX_ORCHESTRATOR_TURNS; turn++) {
    ...
    if (stage === "merge" && await baseHead() !== results.validate!.validatedBaseSha) {
      requestValidation(); await save(); continue;
    }
    if (stage === "merge") await checkCommit(results.validate!);
    ...
    switch (stage) {
      ...
      case "merge": {
        ...
        if (result.commit !== validation.commit || result.validationCommentId !== validation.validationCommentId) throw new Error("merge result differs from the validated commit/comment");
        await git(context.repo, "merge-base", "--is-ancestor", result.commit!, `refs/heads/${context.baseBranch}`);
        if ((await options.client.issue(context.issueId)).state.type !== "completed") throw new Error("merge did not complete the Linear issue");
        finished = { issueId: context.issueId, outcome: "completed", baseBranch: context.baseBranch, commit: result.commit!, validationCommentId: result.validationCommentId!, summary: result.summary };
        reviewReason = "The executor merged the verified commit and completed the issue; review the handoff and return completed, or report a concrete unresolved blocker.";
        break;
      }
    }
    await save();
